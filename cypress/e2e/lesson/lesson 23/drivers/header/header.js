...
class saleButton {
   navigateToMarkdowned(path) {
    clickOnelement(headerSelectors.sale)
    verifyPath(path);
   }
   ...
   cartButtonClick() {
    cy.get(headerSelecters.cart).click();
    cy.location('pathname', {timeout: 4000}).should('include', '/cart/consistence');
   }


   verifyPath(path) {
    cy.location('pathname', {timeout: 4000}).should('include', path);
   }
